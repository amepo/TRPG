...
export class EditorScreen {
  ...
  metaCard() {
    ...
    return el('div', { class: 'card stack' }, [
      el('div', { class: 'spread' }, [
        ...
        button('一覧へ', () => { this.scenario = null; this.render(); }, 'btn btn--sm btn--ghost'),
      ]),
      field('タイトル', el('input', {
      ...
      el('div', { class: 'row' }, [
        ...
        el('div', { style: { width: '110px' } }, [field('想定レベル', el('input', {
          ...
          oninput: e => { s.level = Number(e.target.value) || 1; },
        }))]),
      ]),
      el('div', { class: 'row' }, [
  ...
  nodesCard(nodeIds) {
    return el('div', { class: 'card card--flat stack' }, [
      el('div', { class: 'spread' }, [
        ...
        button('＋ 場面を足す', () => this.addNode(), 'btn btn--sm'),
      ]),
    ...
    return el('div', { class: 'card stack' }, [
      el('div', { class: 'spread' }, [
        ...
        el('div', { class: 'row', style: { gap: '5px' } }, [
          button('複製', () => this.duplicateNode(node), 'btn btn--sm'),
      ...
      field('見出し', el('input', {
        ...
        oninput: e => { node.title = e.target.value; },
      })),
      field('アイコン（絵文字）', el('input', {
        class: 'input', value: node.art || '', maxlength: 4,
        oninput: e => { node.art = e.target.value; },
      })),
  ...
  async removeNode(node) {
    ...
    if (!await confirmSheet('場面を削除', `「${node.title || node.id}」を削除します。ここへのリンクは切れます。`, { danger: true, okText: '削除' })) return;
  ...
  choicesBlock(node, update) {
    ...
    const nodeOptions = (selected, onChange) => el('select', {
      class: 'select', onchange: e => { onChange(e.target.value || undefined); },
    }, [
    ...
    return el('div', { class: 'stack' }, [
      el('div', { class: 'spread' }, [
        ...
        button('＋ 選択肢を足す', () => { node.choices.push({ text: '新しい選択肢', to: this.scenario.start }); update(); }, 'btn btn--sm'),
      ]),
      ...node.choices.map((choice, index) => el('div', { class: 'card card--flat stack', style: { gap: '8px' } }, [
        el('div', { class: 'row' }, [
          el('div', { class: 'grow' }, [el('input', {
            ...
            oninput: e => { choice.text = e.target.value; },
          })]),
          el('button', { class: 'btn btn--sm btn--danger', onclick: () => { node.choices.splice(index, 1); update(); } }, ['×']),
        ]),
        ...
        el('div', { class: 'row' }, [
          ...
          el('label', { class: 'chip' }, [
            el('input', {
              ...
              onchange: e => { choice.once = e.target.checked || undefined; },
            }),
            ' 一度きり',
          ]),
        ]),
          ...
          ? el('div', { class: 'stack', style: { gap: '6px' } }, [
            el('div', { class: 'row' }, [
              el('div', { class: 'grow' }, [field('技能', el('select', {
                class: 'select', onchange: e => { choice.check.skill = e.target.value; },
              ...
              el('div', { style: { width: '120px' } }, [field('DC', el('input', {
                ...
                oninput: e => { choice.check.dc = Number(e.target.value) || 10; },
              }))]),
            ]),
    ...
    return el('div', { class: 'stack' }, [
      ...
      node.combat ? el('div', { class: 'stack', style: { gap: '8px' } }, [
        el('div', { class: 'chips' }, enemies.map((id, index) =>
          el('button', {
            class: 'chip is-on',
            onclick: () => { node.combat.enemies.splice(index, 1); update(); },
          }, [`${MONSTERS[id]?.name || id} ×`]))),
        difficulty ? el('p', { class: 'tiny muted', text: `想定難易度: ${difficulty.name}（XP ${difficulty.xp}）` }) : null,
        el('select', {
          class: 'select',
          onchange: e => { if (e.target.value) { node.combat.enemies.push(e.target.value); update(); } },
        }, [
    ...
    return el('div', { class: 'stack' }, [
      ...
      node.ending ? el('div', { class: 'stack', style: { gap: '8px' } }, [
        ...
        field('結末の見出し', el('input', {
          ...
          oninput: e => { node.ending.title = e.target.value; },
        })),
