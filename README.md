...
**🌃 ネオンの雨（サイバーパンク）**
- **雨の領収書**（40〜60分）— 35場面／戦闘5／侵入1／判定23／結末6。消えた弟を追って企業タワーの37階へ。
  侵入経路は「正面から社員証」「搬入口から潜入」「路地からネットラン」の3通り
...
```bash
npm start          # ローカルサーバー
npm test           # ユニットテスト130件（ダイス・ルール・戦闘・シナリオ／エンジン・世界観）
npm run smoke      # 実ブラウザで3モードと2世界を通しで操作（22項目・要 npm install）
